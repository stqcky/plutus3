...
use factory::{FACTORY_ADDRESS, SushiSwapV2Factory};
use plutus_defi_protocols_protocol::{
    DiscoverableProtocol, Protocol, ProtocolFactory, pool::LiquidityPool,
};
use pool::SushiSwapV2Pool;

#[derive(Clone, Copy)]
pub struct SushiSwapV2Protocol {
    factory: SushiSwapV2Factory,
}

#[async_trait]
impl<P: Provider + Clone + 'static> Protocol<P> for SushiSwapV2Protocol {
...
#[async_trait]
impl<P: Provider + Clone + 'static> DiscoverableProtocol<P> for SushiSwapV2Protocol {
    async fn discover(
        &self,
        from: BlockNumber,
        to: BlockNumber,
...
impl<P: Provider + Clone + 'static> ProtocolFactory<P> for SushiSwapV2Protocol {
    ...
    fn new(chain_id: ChainId) -> Option<Self> {
        ...
        Some(Self {
            factory: SushiSwapV2Factory::new(factory_address),
        })
    }
}
